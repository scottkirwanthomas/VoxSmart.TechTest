...
{
    public class MetalCommoditiesStrategy : IExtractionStrategy
    {
         public MetalCommoditiesStrategy(IOptions<ExtractionConfiguration> options)
        {
            IsEnabled = options.Value.EnabledStrategies.Contains(nameof(MetalCommoditiesStrategy));
        }
        ...
        public List<string> ExtractEntity(List<MarketItem> items)
        {
            ...
            StaticData.MetalCommodities.ForEach((x) =>
            {
